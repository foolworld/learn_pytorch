...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 32, 3, 1)  # 输入1通道，输出32通道，3x3卷积
    ...
    def forward(self, x):
        x = self.relu(self.conv1(x))  # [64, 32, 26, 26]
        x = self.maxpool(x)  # [64, 32, 13, 13]
        x = self.relu(self.conv2(x))  # [64, 64, 11, 11]
        x = self.maxpool(x)  # [64, 64, 5, 5]
        x = x.view(x.size(0), -1)  # [64, 1600]
        x = self.relu(self.fc1(x))  # [64, 128]
        x = self.fc2(x)  # [64, 10]
        return x
